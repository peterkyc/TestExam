/* eslint-disable @typescript-eslint/consistent-type-assertions */
import { useState, useCallback } from 'react';
import { useDispatch } from 'react-redux';
import { AuthProvider, showNotification, resetNotification, useAuthProvider } from 'react-admin';
import { useHistory } from 'react-router-dom';
import { login, register, logout, log, error } from "./api/request";
import { Status } from "../common/utils";
import { NotificationOptions } from "./pages/utils";
import { ThemeName } from './types';

type Request = (params: any, pathName?: string) => Promise<any>;

const User = "userData";
const Theme = "theme";
export interface UserData {
  userId: string;
  password?: string;
  name?: string;
  type?: string;
  imageUrl?: string;
  status?: string | number;
  [index: string]: any;
}
export function getTheme(): ThemeName {
  const defTheme = "light";
  try {
    let ret: ThemeName = <ThemeName>localStorage.getItem(Theme);
    return (!ret || ["light", "dark"].indexOf(ret) < 0) ? defTheme : ret;
  } catch (e) {
  }
  return defTheme;
}
export function setTheme(theme: ThemeName) {
  try {
    localStorage.setItem(Theme, theme);
  } catch (e) {
  }
  return theme;
}
export function getUserData(): UserData {
  try {
    let ret = localStorage.getItem(User);
    return ret && JSON.parse(ret);
  } catch (e) {
  }
  return null;
}
export function setUserData(data: UserData | null, imageUrl?: string) {
  try {
    if (data === null) {
      localStorage.removeItem(User);
    } else {
      if (imageUrl) {
        data.imageUrl = imageUrl;
      }
      localStorage.setItem(User, JSON.stringify(data));
    }
  } catch (e) {
  }
  return data;
}
export const removeUserData = () => setUserData(null);


export const useRegister = (): Request => {
  const history = useHistory();
  const dispatch = useDispatch();
  const authProvider = useAuthProvider();
  const registerCB = (params: any = {}, pathName) => {
    let { type } = params || {};
    return authProvider.register(params).then(ret => {
      // if(ret?.message) {
      //   showNotification(ret?.message,"error");
      // }
      if (ret.status == Status.UserExists) {
        history.push("/login");
      } else if (ret.status == Status.VerifyEMail) {
        history.push("/resendMail");
      } else {
        history.push("/");
      }
      dispatch(resetNotification());
      return ret;
    });
  }
  const register = useCallback(registerCB, [authProvider, history, dispatch]);
  return register;
};
export const useLogin = (): Request => {
  const history = useHistory();
  const dispatch = useDispatch();
  const authProvider = useAuthProvider();
  const loginCB = (params: any = {}, pathName) => {
    return authProvider.login(params).then(ret => {
      if (ret.status == Status.UserNotFound) {
        history.push("/login?page=register");
      } else if (ret.status == Status.VerifyEMail) {
        history.push("/resendMail");
      } else {
        dispatch(resetNotification());
        history.push("/");
      }
      return ret;
    });
  }
  const login = useCallback(loginCB, [authProvider, history, dispatch]);
  return login;
};

// const ShowMessageAndReply = [Status.UserExists, Status.VerifyEMail];


const authProvider: AuthProvider = {
  register: (data) => {
    const { imageUrl } = data;
    return register(data).then((ret = {}) => {
      let status = ret?.status;
      if (Status.UserExists === status) {
        return Promise.resolve(ret);
      } else if (Status.VerifyEMail === status) {
        return Promise.resolve(setUserData(ret, imageUrl));
      } else if (status !== 0) {
        let message = (ret && ret.message) || "response fail data";
        showNotification(message, "error", NotificationOptions);
        return Promise.reject(message);
      }
      return Promise.resolve(setUserData(ret, imageUrl));
    });
  },
  login: (data) => {
    const { imageUrl } = data;
    return login(data).then(ret => {
      let status = ret?.status;
      if (Status.VerifyEMail === status) {
        return Promise.resolve(setUserData(ret, imageUrl));
      } else if (status !== 0) {
        let message = (ret && ret.message) || "response fail data";
        showNotification(message, "error", NotificationOptions);
        if (ret.status !== Status.UserNotFound) {
          return Promise.reject(message);
        }
      } else {
        setUserData(ret, imageUrl);
      }
      return Promise.resolve(ret);
    });
  },
  logout: () => {
    const ud = getUserData(), { userId } = ud || {};
    logout({ userId }).then(ret => {
      log("logout", "OK", ret);
    }).catch(err => {
      error("logout", "OK", err);
    });
    removeUserData();
    // localStorage.removeItem('username');
    return Promise.resolve();
  },
  checkError: () => {
    return Promise.resolve();
  },
  checkAuth: () => {
    let ret = getUserData(); //localStorage.getItem('username');
    return ret ? Promise.resolve() : Promise.reject();
  },
  getPermissions: () => {
    return Promise.reject('Unknown method');
  },
  getIdentity: () => {
    const avatar = 'data:image/jpeg;base64,';
    const ud = getUserData(), { userId: id, name: fullName, imageUrl } = ud || {};
    let data = { id, fullName:"", avatar };
    imageUrl && (data.avatar = imageUrl);
    return Promise.resolve(data);
  },
  getUserData() {
    const data = getUserData();
    return data;
    // return Promise.resolve(data);
  },
  setUserData(data) {
    data = setUserData(data);
    return data;
    // return Promise.resolve(data);
  }
};

export default authProvider;
