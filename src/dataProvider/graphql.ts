...
const url = getUrl(), uri = `${url}/api/test/graphql`;
  ...
  return new Proxy<DataProvider>(defaultDataProvider, {
    get: (target, name) => {
      ...
      return async (resource: string, params: any) => {
        let ret, func = dataProvider[name], gqlRes = getGqlResource(resource);
        console.log(`gqlRes ${resource},${gqlRes} params:`, params);
        try {
          ret = func(gqlRes, params);
          console.log(`resp ${resource}:`, ret);
        } catch (e) {
          console.error(``, e);
        }
        return ret;
