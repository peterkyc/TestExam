...
const { origin } = window.location; //pathJoin(origin, path);
export const getApi = (api?) => api ? `/api/test/${api}` : "";

export const getUrl = (api?) => `${origin}${getApi(api)}`;
    ...
    get(_, name) {
      return (resource: string, params: any) => {
        return dataProviderPromise.then(dataProvider => {
          const func = dataProvider[name.toString()];
